...
uint32_t BID_PRICE = 400'000;
uint32_t ASK_PRICE = 500'000;

std::set<uint32_t> BIDS = {BID_PRICE};
std::set<uint32_t> ASKS = {ASK_PRICE};
...
TEST(OrderBook, bid_ask) {
   ...
   OrderBook book{orders, BIDS, ASKS};
...
TEST(OrderBook, add_delete) {
   ...
   OrderBook book{orders, BIDS, EMPTY};
   book.OrderAdd(ItchOrderAdd{{1, 1, 0}, 1, 1, BID_PRICE});
...
TEST(OrderBook, add_replace) {
   ...
   std::set<uint32_t> bids = {BID_PRICE, new_price};
   OrderBook book{orders, bids, EMPTY};
   book.OrderAdd(ItchOrderAdd{{1, 1, 0}, 1, 1, BID_PRICE});
